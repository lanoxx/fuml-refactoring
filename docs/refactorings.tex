...
In this paper we will present a set of refactorings and give examples of how each refactoring can be applied to a concrete model \todo{it seems we are repeating our selfs about what we will present}.
...
In this section\todo{Seems to be a misplaced section} we will present some general refactorings such as the ``extract superclass'' refactoring. A list of example refactorings
...
\textit{extract class} and the \textit{rename class}, \textit{rename method} and \textit{rename variable} refactorings.
...
Refactoring models is a rather difficult task and it requires a consise knowledge of the involved technologies, with
...
In particular this meant to identify how the activity diagrams needs to change if a change is made in the class
diagram.

Most of our efforts in the development of this work were focused on gaining a comprehensive understanding of the
technologies described above, to develop the tool chain and to draw the various diagrams presented in this paper. We
have presented a comprehensive set of diagrams as the basis for our refactoring work and demonstrated the feasibility
of our tool chain. However a more comprehensive evaluation of the different refactorings including the specification
of pre- and postconditions OCL in is still required and will be the focus of our ongoing efforts.
