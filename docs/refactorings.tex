...
trivial and requires additional considerations, which are not covered by this refactoring.
...
operation} afterwards. There are two cases of the \textit{pull up property} refactoring. It can either be executed for a property that 
occurs in a single class or in multiple classes where the properties should also be combined and pulled up. In the class diagram 
...
The models are stored in \textit{XML Metadata Interchange} \cite{man:XMI} (XMI) format 
and loaded programmatically with an \textit{Eclipse Modeling Framework} \cite{Steinberg:2009:EEM:1197540} (EMF) 
\texttt{Resource\-Set}. The \texttt{Resource\-Set}
can be used to retrieve a \texttt{Resource} object through an \texttt{URI}, which is then used to access the different objects in the model 
