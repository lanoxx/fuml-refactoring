...
sections below we describe some examples.
...
and can be associated with these two classes through the recursive attribute \textit{superclass} of the class \textit{Class} 
to create the inheritance hierarchy. So the meta-model representation of the diagram does not change. However in order to 
guarantee that the refactoring is possible, we must ensure that each class that participates in the refactoring does not 
already have a superclass. The model would also benefit from changing all possible references to the new class.

\subsection{Pull up attribute}
\label{sec:pullup}
There are two cases of the \textit{pull up method} refactoring. It can either be executed for an attribute that occurs in 
...
the super class. A precondition for this refactoring is that there does not already exist an attribute with the same name 
in the target class and that the field is not private.
