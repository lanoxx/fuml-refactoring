...
also receives more and more attention from the industry. Nowadays models are not only used as visual explanations of
...
The post conditions for rename attribute apply to all \lstinline|StructuralFeatureAction| objects
which reference the renamed attribute. For rename operation it applies to all \lstinline|CallOperationAction| objects which
...
interface that has three methods \lstinline|checkPrecondition|, \lstinline|performRefactoring|, and \lstinline|checkPostcondition|. 
...
the refactoring did not corrupt the model. Further testing can be implemented by going through the trace and verifying 
...
which focuses on static model analysis refactoring.
